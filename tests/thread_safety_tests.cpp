...
public:
  ...
  database_types database_type() override { return type_; }
...
TEST_F(DatabaseThreadSafetyTest, ConcurrentConnectionAcquireRelease) {
  ...
  // With mock connections returning nullptr, all acquisitions should fail
  // gracefully
...
TEST_F(DatabaseThreadSafetyTest, ConnectionPoolStatsAccess) {
  ...
  connection_pool pool(database_types::postgres, config,
...
TEST_F(DatabaseThreadSafetyTest, ConnectionPoolManagerConcurrentCreation) {
  GTEST_SKIP() << "Skipped: connection_pool_manager uses real database "
                  "backends (PostgreSQL/MySQL) which are not available in CI";
    ...
    threads.emplace_back([&, thread_id = i]() {
      ...
      try {
        ...
        database_types db_type =
            static_cast<database_types>((thread_id % 3) + 1);
...
TEST_F(DatabaseThreadSafetyTest, PoolManagerGetRemoveRace) {
  GTEST_SKIP() << "Skipped: connection_pool_manager uses real database "
                  "backends which are not available in CI";
  ...
  std::this_thread::sleep_for(
      50ms); // Reduced from 200ms - minimum wait for test completion
  running.store(false);
...
TEST_F(DatabaseThreadSafetyTest, DatabaseManagerConcurrentSetMode) {
  GTEST_SKIP() << "Skipped: database_manager::set_mode() initializes real "
                  "database backends which causes SegFault in CI";
...
TEST_F(DatabaseThreadSafetyTest, ConnectionWrapperMetadataConcurrent) {
  ...
  auto mock_conn =
      std::make_unique<connection_wrapper>(create_mock_connection());
...
TEST_F(DatabaseThreadSafetyTest, PoolLifecycleMemorySafety) {
  ...
  for (int iteration = 0; iteration < num_iterations; ++iteration) {
    ...
    connection_pool pool(database_types::sqlite, config,
                         create_mock_connection);
    pool.initialize();
