...
protected:
  void SetUp() override { manager_ = std::make_unique<replication_manager>(); }
...
TEST_F(SafeQueryBuilderTest, InvalidIdentifier) {
  ...
  EXPECT_FALSE(
      safe_query_builder::is_valid_identifier("user-name")); // contains hyphen
  EXPECT_FALSE(
      safe_query_builder::is_valid_identifier("user name")); // contains space
  EXPECT_FALSE(safe_query_builder::is_valid_identifier(
      "user;name")); // contains semicolon
  EXPECT_FALSE(
      safe_query_builder::is_valid_identifier("user'name")); // contains quote
}

TEST_F(SafeQueryBuilderTest, EscapeIdentifier) {
  EXPECT_EQ(safe_query_builder::escape_identifier("users"), "\"users\"");
  EXPECT_EQ(safe_query_builder::escape_identifier("user_name"),
            "\"user_name\"");
}

TEST_F(SafeQueryBuilderTest, EscapeIdentifierThrowsOnInvalid) {
  EXPECT_THROW(safe_query_builder::escape_identifier("user;DROP"),
               std::invalid_argument);
  EXPECT_THROW(safe_query_builder::escape_identifier(""),
               std::invalid_argument);
}
...
TEST_F(SafeQueryBuilderTest, BuildInsertThrowsOnEmptyTable) {
  std::unordered_map<std::string, std::string> values = {{"name", "John"}};
  EXPECT_THROW(safe_query_builder::build_insert("", values),
               std::invalid_argument);
}

TEST_F(SafeQueryBuilderTest, BuildInsertThrowsOnEmptyValues) {
  std::unordered_map<std::string, std::string> values;
  EXPECT_THROW(safe_query_builder::build_insert("users", values),
               std::invalid_argument);
}
...
TEST_F(SafeQueryBuilderTest, BuildDeleteThrowsOnEmptyWhere) {
  ...
  EXPECT_THROW(safe_query_builder::build_delete("users", where_values),
               std::invalid_argument);
}
...
TEST_F(CDCFactoryTest, DetectPostgreSQLFromConnectionString) {
  EXPECT_EQ(cdc_factory::detect_database_type(
                "postgresql://user:pass@localhost:5432/db"),
            database_type::POSTGRESQL);
  EXPECT_EQ(
      cdc_factory::detect_database_type("postgres://user:pass@localhost/db"),
      database_type::POSTGRESQL);
}

TEST_F(CDCFactoryTest, DetectMySQLFromConnectionString) {
  EXPECT_EQ(
      cdc_factory::detect_database_type("mysql://user:pass@localhost:3306/db"),
      database_type::MYSQL);
}
...
TEST_F(CDCFactoryTest, GetTypeName) {
  ...
  EXPECT_EQ(cdc_factory::get_type_name(database_type::POSTGRESQL),
            "PostgreSQL");
...
protected:
  void SetUp() override { strategy_ = std::make_unique<sqlite_cdc_strategy>(); }
...
TEST_F(SQLiteCDCStrategyTest, InitializeWithMemoryDatabase) {
  ...
  // This will fail because the table doesn't exist, but initialization
  // structure works
...
protected:
  void SetUp() override { strategy_ = std::make_unique<mysql_cdc_strategy>(); }
...
TEST_F(MySQLCDCStrategyTest, PositionManagement) {
  ...
  // Test GTID format
  result =
      strategy_->set_position("gtid:3E11FA47-71CA-11E1-9E33-C80AA9429562:1-5");
  EXPECT_TRUE(result.is_ok());
}
...
TEST_F(TargetClientInitializationTest, DetectPostgreSQLFromConnectionString) {
  auto db_type = cdc_factory::detect_database_type(
      "postgresql://user:pass@localhost:5432/db");
...
TEST_F(TargetClientInitializationTest, StartReplicationWithSQLiteTarget) {
  ...
  if (result.is_err()) {
    ...
    EXPECT_TRUE(result.error().message.find("backend") != std::string::npos ||
...
TEST_F(TargetClientInitializationTest, TargetWithDirectHostConfig) {
  ...
  if (result.is_err()) {
    ...
    EXPECT_TRUE(result.error().message.find("backend") != std::string::npos ||
                result.error().message.find("connection") !=
                    std::string::npos ||
                result.error().message.find("initialize") != std::string::npos);
  }
}
...
TEST_F(ReplicationManagerTest, ObservabilityWithFileLogging) {
  std::filesystem::path temp_dir =
      std::filesystem::temp_directory_path() / "replication_observability_test";
