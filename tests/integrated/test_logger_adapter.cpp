...
bool test_initialization()
{
	...
	TEST_EXPECT_TRUE(init_result.is_ok(), "Logger should initialize successfully");
	TEST_EXPECT_TRUE(logger.is_initialized(), "Logger should be marked as initialized");
	...
	TEST_EXPECT_TRUE(shutdown_result.is_ok(), "Logger should shutdown successfully");
	TEST_EXPECT_FALSE(logger.is_initialized(), "Logger should not be initialized after shutdown");
...
bool test_query_logging()
{
	...
	logger.log_query(db_log_level::info,
		"CREATE USER 'admin' IDENTIFIED BY PASSWORD 'secret123'", std::chrono::microseconds(500));
	...
	logger.log_query(db_log_level::info, long_query, std::chrono::microseconds(2000));
...
bool test_slow_query_detection()
{
	...
	logger.log_query(
		db_log_level::info, "SELECT * FROM fast_table", std::chrono::microseconds(100000)); // 100ms

	// Test slow query (should trigger warning)
	logger.log_query(
		db_log_level::info, "SELECT * FROM slow_table", std::chrono::microseconds(600000)); // 600ms

	// Explicit slow query log
	logger.log_slow_query("SELECT * FROM very_slow_table", std::chrono::microseconds(1200000),
...
bool test_transaction_logging()
{
	...
	logger.log_transaction("rollback", false, "Constraint violation: duplicate key");
