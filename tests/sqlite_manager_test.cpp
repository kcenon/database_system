...
protected:
  ...
  bool createTestTable() {
    return manager_->execute_query("CREATE TABLE IF NOT EXISTS test_table ("
...
TEST_F(SQLiteManagerTest, SelectAllColumns) {
  ...
  manager_->insert_query("INSERT INTO test_table (name, value, active) VALUES "
                         "('test_item', 42.5, 1)");
...
TEST_F(SQLiteManagerTest, SelectWithCondition) {
  ...
  auto result =
      manager_->select_query("SELECT * FROM test_table WHERE value > 15.0");
...
#ifdef USE_SQLITE
  EXPECT_TRUE(manager_->connect(":memory:"));
  EXPECT_TRUE(manager_->execute_query("CREATE TABLE int_test (val INTEGER)"));
...
#ifdef USE_SQLITE
  EXPECT_TRUE(manager_->connect(":memory:"));
  EXPECT_TRUE(manager_->execute_query("CREATE TABLE float_test (val REAL)"));
...
#ifdef USE_SQLITE
  EXPECT_TRUE(manager_->connect(":memory:"));
  EXPECT_TRUE(manager_->execute_query("CREATE TABLE null_test (val TEXT)"));
...
TEST_F(SQLiteManagerTest, UpdateSingleRow) {
  ...
  auto result =
      manager_->select_query("SELECT name FROM test_table WHERE id = 1");
  ASSERT_EQ(result.size(), 1u);
...
TEST_F(SQLiteManagerTest, UpdateMultipleRows) {
  ...
  manager_->insert_query(
      "INSERT INTO test_table (name, value, active) VALUES ('item1', 10.0, 1)");
  manager_->insert_query(
      "INSERT INTO test_table (name, value, active) VALUES ('item2', 20.0, 1)");
  manager_->insert_query(
      "INSERT INTO test_table (name, value, active) VALUES ('item3', 30.0, 0)");
...
TEST_F(SQLiteManagerTest, DeleteSingleRow) {
  ...
  unsigned int affected =
      manager_->delete_query("DELETE FROM test_table WHERE id = 1");
  EXPECT_EQ(affected, 1u);

  auto result =
      manager_->select_query("SELECT COUNT(*) as cnt FROM test_table");
  ASSERT_EQ(result.size(), 1u);
...
TEST_F(SQLiteManagerTest, ExecuteDropTable) {
  ...
  auto result = manager_->select_query("SELECT name FROM sqlite_master WHERE "
                                       "type='table' AND name='temp_table'");
  EXPECT_TRUE(result.empty());
#else
  EXPECT_FALSE(manager_->connect(":memory:"));
#endif
...
TEST_F(SQLiteManagerTest, BlobDataHandling) {
  ...
  auto result =
      manager_->select_query("SELECT data FROM blob_test WHERE id = 1");
  ASSERT_EQ(result.size(), 1u);
...
TEST_F(SQLiteManagerTest, ForeignKeyConstraint) {
  ...
  EXPECT_TRUE(
      manager_->execute_query("CREATE TABLE child ("
  ...
  unsigned int result =
      manager_->insert_query("INSERT INTO child VALUES (2, 999)");
  EXPECT_EQ(result, 0u);
#else
  EXPECT_FALSE(manager_->connect(":memory:"));
#endif
