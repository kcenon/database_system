...
TEST_F(ConnectionStressTest, ConcurrentConnectionCreation) {
  ...
  EXPECT_EQ(successful.load(), total)
      << "Failed concurrent connection creation: " << failed.load()
      << " failures";
#else
  GTEST_SKIP() << "SQLite not available";
#endif
...
TEST_F(ConnectionStressTest, PoolExhaustionRecovery) {
  ...
  try {
    ...
    EXPECT_TRUE(exhaustion_handled ||
                acquired_count < static_cast<int>(config.max_connections))
...
TEST_F(ConnectionStressTest, NoLeaksAfterExceptions) {
  ...
  SUCCEED() << "Completed " << ITERATIONS
            << " iterations with exception handling";
#else
  GTEST_SKIP() << "SQLite not available";
#endif
}
